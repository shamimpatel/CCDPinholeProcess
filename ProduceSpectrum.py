...
Outfile = open('CCDSpectrumDiffract.txt', 'w')
...
for line in open("DiffractResultsPostPinhole.txt"):
	XRay = line.split('\t')
...
Outfile = open('CCDSpectrumFluo.txt', 'w')

for line in open("FluoResultsPostPinhole.txt"):
	XRay = line.split('\t')
	if (0 <= int(XRay[0]) < numXPixels) and (0 <= int(XRay[1]) < numYPixels):
		Energy = float(XRay[2])*1000; #energy in ev
		#if(Energy < 3000.0):
...
print "Output Photons (without CCD Misses ): ", nOutputPhotons
